...
//  Created by Anupriya on 13/12/17.
...
let appNameUIPhotosButtonFromGallery = Bundle.main.infoDictionary![kCFBundleNameKey as String] as! String

let rootFolder1: String = "\(NSTemporaryDirectory())UIMultiplePhoto/"

class UIPhotosButtonFromGallery: UIButton, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    public  var closureDidFinishPickingAnImage: ((_ image: [String]) -> Void)?
     ...
     public  var closureDidFinishPickingUIImage: ((_ image: [UIImage]) -> Void)?
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        print(UIPhotosButtonFromGallery.photoPath(), NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true)[0] as String, rootFolder1)
    ...
    class func removeCache() {
        ...
        do {
            try fileManger.removeItem(atPath: rootFolder1)
    ...
    private class func photoPath() -> String {
        ...
        if !fileManger.fileExists(atPath: rootFolder1) {
            do {
                try fileManger.createDirectory(atPath: "\(rootFolder1)", withIntermediateDirectories: false, attributes: nil)
    ...
    @objc private func addPhoto() {
        ...
        PhotoAlertHelper.alertView(title: appNameUIPhotosButtonFromGallery,
                                   message: "Select image.",
                                   preferredStyle: .actionSheet,
                                   cancelTilte: "Cancel",
                                   otherButtons: "Gallery",
    ...
    @objc func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any]) {
        ...
        let filePath = URL(fileURLWithPath: UIPhotosButtonFromGallery.photoPath() + "\(NSUUID().uuidString)").appendingPathExtension("jpg")
        ...
        imagePaths.append(filePath.path)
			...
			if let  image = info[UIImagePickerController.InfoKey.originalImage] as?  UIImage {
                ...
                print("🥚size of image in Mb: %f ", Double(imageSize) / (1024.0*1024.0),image)
				do {
					try imageData?.write(to: filePath, options: .atomic)

                    closureDidFinishPickingUIImage?([image])
...
class GalleryAlertHelper: UIAlertController {
